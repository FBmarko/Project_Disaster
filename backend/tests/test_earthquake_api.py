...
pytestmark = [
    ...
    pytest.mark.usefixtures("sample_earthquake_dataset"),
]
...
def test_api_read_operations_leave_database_clean() -> None:
    ...
    assert f_count_before is not None and f_count_before >= 0
    assert f_count_after == f_count_before
    assert q_count_before is not None and q_count_before >= 0
    assert q_count_after == q_count_before


def test_rolling_window_read_time_enforcement(isolated_session: Session) -> None:
    ...
    assert all(f.properties.occurred_at >= tighter_start for f in res_tighter.features)
