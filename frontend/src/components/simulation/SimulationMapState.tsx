  ...
  return (
    <div className="flex h-full w-full items-center justify-center bg-surface p-6 text-center"
      ...
      <div className="max-w-sm">
        <span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-border-subtle bg-white">
          <Icon size={26} aria-hidden="true" className={state === 'loading' ? 'text-brand-red motion-safe:animate-spin' : 'text-brand-red'} />
        </span>
        ...
        {state === 'error' ? <button type="button" onClick={() => window.location.reload()}
          className="mt-4 min-h-11 rounded-lg border border-border-subtle bg-white px-4 text-sm font-medium hover:bg-brand-red-soft">Sayfayı Yenile</button> : null}
      </div>
    </div>
  )
}
