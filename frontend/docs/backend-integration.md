...
| Preparedness | `/api/v1/ai/preparedness-guide` | POST | AVAILABLE (AI-backed) | End-to-end integration via Vite proxy/backend API. Form maps household profile fields; structured response provides canonical guidance sections. |
...
- **Preparedness request:** `{disaster_type: earthquake|flood|fire, city?: string|null, language?: tr|en, household_size: 1..20, has_children: bool, has_elderly_person: bool, has_pets: bool}`; city trimmed, 1–80 characters, language defaults tr, household fields default to 1, false, false, false; extra fields forbidden. **Response:** `{disaster_type, city, language, generated_by_ai, guide, disclaimer}`. Guide: `summary` 10–600 chars; `priorities` 1–8 strings; `emergency_kit` 1–12 strings; `communication_plan` 1–8 strings; `special_needs` 0–8 strings; `important_notes` 0–6 strings. 200 with active Gemini provider; 422 invalid request, 502 provider/malformed output, 503 provider unavailable. No DB needed. Client maps to camelCase display types; error codes map to user-friendly Turkish notices.
...
`.env.example` documents `VITE_API_BASE_URL` (optional for local Vite dev). Map visualization uses MapLibre GL JS with OpenFreeMap and requires no API key. No AI credentials or server secrets exist in frontend configuration. Local ignored environment files remain uncommitted.
